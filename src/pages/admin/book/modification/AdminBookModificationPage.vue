...
const BOOK_MODIFICATION_SUCCESS_MESSAGE = "도서가 수정됐습니다";
    ...
    await AdminBookApis.patchBook(
      ...
      .then(() => {
        confirmModalInfo.value = {
          message: BOOK_MODIFICATION_SUCCESS_MESSAGE,
          buttonText: "확인",
          visible: true,
        };
      })
      .catch((error) => {
        console.error("AdminBookModificationPage.modifyBook", error);
  ...
  await BookApis.getBook(bookId)
    .then((response) => {
      bookPatchRequest.value = {
        ...
        isIndie: response.isIndie,
        isPermitted: true,
        coverImageFilename: response.coverImageUrl,
      } as BookPatchRequest;
    })
    .catch((error) =>
      console.error("AdminBookModificationPage.fetchBook", error)
    );
};
