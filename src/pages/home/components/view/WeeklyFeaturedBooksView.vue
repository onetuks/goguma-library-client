...
import WeeklyFeaturedSelectedBookDetailView from "@/pages/home/components/view/SelectedBookDetailView.vue";
...
const fetchWeeklyFeaturedBooks = async (): Promise<void> => {
  await BookApis.getWeeklyFeaturedBooks()
    ...
    .catch((error) =>
      console.error("WeeklyFeaturedBooksView.fetchWeeklyFeaturedBooks", error)
    );
};
...
<template>
  <div class="weekly-featured-books-container">
    ...
    <WeeklyFeaturedSelectedBookDetailView
      :book="selectedBook"
      v-if="selectedBook"
    />
  </div>
